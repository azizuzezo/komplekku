...
export function AccountView() {
  ...
  const statusColors: Record<string, React.CSSProperties> = {
    success: { background: "oklch(92% 0.045 155.85)", color: "oklch(42% 0.08 155.85)", border: "1px solid oklch(80% 0.07 155.85)" },
    warning: { background: "oklch(93% 0.05 70.45)", color: "oklch(43% 0.105 70.45)", border: "1px solid oklch(82% 0.08 70.45)" },
    danger: { background: "oklch(93% 0.05 23.49)", color: "oklch(45% 0.13 23.49)", border: "1px solid oklch(82% 0.08 23.49)" },
    muted: { background: "rgba(255,255,255,0.12)", color: "#B2EBF2", border: "1px solid rgba(255,255,255,0.2)" },
  };

  const StatusBadge = ({ tone, label }: { tone: string; label: string }) => (
    <span style={{ display: "inline-flex", alignItems: "center", fontSize: "0.68rem", fontWeight: 800, padding: "0.25rem 0.75rem", borderRadius: "9999px", letterSpacing: "0.02em", ...statusColors[tone] }}>
      {label}
    </span>
  );
  ...
  const EditNameBtn = ({ onClick }: { onClick: () => void }) => (
    <button
      ...
      style={{
        ...
        fontSize: "0.7rem", fontWeight: 700, color: "#00ACC1",
        background: "#E0F7FA", border: "1px solid #80DEEA",
        borderRadius: "9999px", padding: "0.22rem 0.65rem", cursor: "pointer",
      }}
    >
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.125rem" }}>
      ...
      {hasResidentCredential ? (
        <section aria-labelledby="account-name" style={CARD}>
          ...
          <div style={{ padding: "1.25rem 1.375rem", background: "#ffffff" }}>
            <div style={{ marginBottom: "1rem" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.3rem" }}>
                <p style={{ fontSize: "0.65rem", fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", color: "#376E76", margin: 0 }}>
                  Nama warga
                </p>
                {!isEditingName && (
                  <EditNameBtn onClick={() => { setNewDisplayName(displayName); setIsEditingName(true); setNameError(null); }} />
                )}
              </div>
                ...
                <form onSubmit={handleSaveName} style={{ marginTop: "0.375rem" }}>
                  <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
                    ...
                    <button
                      ...
                      title="Simpan"
                      style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: "2.25rem", height: "2.25rem", borderRadius: "0.5rem", border: "none", background: "#00ACC1", color: "#fff", cursor: "pointer", flexShrink: 0 }}
                    >
                      <Check size={16} />
                    </button>
                    <button
                      ...
                      title="Batal"
                      style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: "2.25rem", height: "2.25rem", borderRadius: "0.5rem", border: "1px solid #80DEEA", background: "#fff", color: "#376E76", cursor: "pointer", flexShrink: 0 }}
                    >
                      <X size={16} />
                    </button>
                  </div>
                  {nameError && (
                    <p style={{ fontSize: "0.72rem", color: "oklch(45% 0.13 23.49)", marginTop: "0.35rem" }}>{nameError}</p>
                  )}
                </form>
              ) : (
                <h2
                  id="account-name"
                  style={{ fontSize: "1.875rem", fontWeight: 800, color: "#0F2F34", letterSpacing: "-0.035em", lineHeight: 1.1, margin: 0 }}
                >
                  {displayName}
                </h2>
              )}

              <span style={{ fontSize: "0.73rem", fontFamily: "monospace", color: "#376E76", marginTop: "0.25rem", display: "block" }}>
                {account.phoneMasked}
              </span>
            </div>

            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", borderRadius: "0.75rem", overflow: "hidden", border: "1px solid #80DEEA" }}>
              {[
                ...
                <div
                  key={item.label}
                  style={{ padding: "0.75rem 0.875rem", background: "#E0F7FA", borderLeft: i > 0 ? "1px solid #80DEEA" : undefined }}
                >
                  <p style={{ fontSize: "0.6rem", fontWeight: 700, color: "#376E76", textTransform: "uppercase", letterSpacing: "0.06em", margin: "0 0 0.2rem" }}>
                    {item.label}
                  </p>
                  <p style={{ fontSize: "0.8rem", fontWeight: 800, color: "#0F2F34", margin: 0, lineHeight: 1.25 }}>{item.value}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={FOOT}>
            <span style={{ fontSize: "0.68rem", color: "#4DD0E1", fontWeight: 600 }}>{context.community.name}</span>
            <strong style={{ fontSize: "0.8rem", fontFamily: "monospace", color: "#4DD0E1", fontWeight: 800, letterSpacing: "0.1em" }}>
        ...
        <section aria-labelledby="account-name" style={CARD}>
          <div style={HERO}>
            <BrandMark variant="mark" href={null} />
            {residentStatus && <StatusBadge tone={statusTone} label={residentStatusLabels[residentStatus] ?? residentStatus} />}
          </div>
          <div style={{ padding: "1.25rem 1.375rem", background: "#ffffff" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.5rem" }}>
              ...
              {!isEditingName && <EditNameBtn onClick={() => { setNewDisplayName(displayName); setIsEditingName(true); }} />}
            </div>
            {isEditingName ? (
              <form onSubmit={handleSaveName} style={{ display: "flex", gap: "0.5rem" }}>
                <input type="text" value={newDisplayName} onChange={(e) => setNewDisplayName(e.target.value)} className="input" placeholder="Nama baru" style={{ flex: 1, color: "#0F2F34" }} />
                <button type="submit" style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: "2.25rem", height: "2.25rem", borderRadius: "0.5rem", border: "none", background: "#00ACC1", color: "#fff", cursor: "pointer", flexShrink: 0 }}>
                  <Check size={16} />
                </button>
              </form>
            ) : (
              <h2 id="account-name" style={{ fontSize: "1.5rem", fontWeight: 800, color: "#0F2F34", margin: 0 }}>{displayName}</h2>
            )}
            <p style={{ fontSize: "0.73rem", fontFamily: "monospace", color: "#376E76", marginTop: "0.25rem" }}>{account.phoneMasked}</p>
          </div>
        </section>
      )}
        ...
        <section style={SECTION_CARD}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1rem" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
              <div style={{ width: "2.25rem", height: "2.25rem", borderRadius: "0.625rem", background: "#E0F7FA", display: "flex", alignItems: "center", justifyContent: "center", color: "#00ACC1" }}>
                <Users size={18} />
              </div>
              <div>
                <h3 style={{ fontSize: "0.9rem", fontWeight: 800, color: "#0F2F34", margin: 0 }}>Anggota Keluarga</h3>
                <p style={{ fontSize: "0.7rem", color: "#376E76", margin: 0 }}>Penghuni rumah tangga ini</p>
              </div>
            </div>
            {!isAddingFamily && (
              <button
                ...
                style={{ display: "inline-flex", alignItems: "center", gap: "0.3rem", fontSize: "0.75rem", fontWeight: 700, color: "#fff", background: "#00ACC1", border: "none", borderRadius: "0.625rem", padding: "0.45rem 0.875rem", cursor: "pointer" }}
              >
                <Plus size={14} /> Tambah
              </button>
            )}
          ...
          {isAddingFamily && (
            <div style={{ background: "#F3FBFC", border: "1px solid #80DEEA", borderRadius: "0.75rem", padding: "1rem", marginBottom: "0.75rem" }}>
              <h4 style={{ fontSize: "0.75rem", fontWeight: 700, color: "#00ACC1", display: "flex", alignItems: "center", gap: "0.35rem", margin: "0 0 0.75rem" }}>
      ...
      {account.permissions.includes("resident.manage") && (
        <section aria-labelledby="account-admin-heading" style={SECTION_CARD}>
          <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
            <div style={{ width: "2.5rem", height: "2.5rem", borderRadius: "0.75rem", background: "#E0F7FA", display: "flex", alignItems: "center", justifyContent: "center", color: "#00ACC1", flexShrink: 0 }}>
              <ClipboardCheck size={20} />
            </div>
            <div style={{ flex: 1 }}>
              <h2 id="account-admin-heading" style={{ fontSize: "0.875rem", fontWeight: 700, color: "#0F2F34", margin: 0 }}>
                Tugas pengurus
              </h2>
              <p style={{ fontSize: "0.75rem", color: "#376E76", margin: 0 }}>Tinjau permohonan tempat tinggal.</p>
            </div>
            <Link
              href="/admin/permohonan-warga"
              style={{ display: "inline-flex", alignItems: "center", gap: "0.25rem", fontSize: "0.75rem", fontWeight: 700, color: "#00ACC1", textDecoration: "none" }}
            >
      ...
      <section aria-labelledby="account-session-heading" style={SECTION_CARD}>
        <h2 id="account-session-heading" style={{ fontSize: "0.875rem", fontWeight: 700, color: "#0F2F34", margin: "0 0 0.25rem" }}>
          Sesi akun
        </h2>
        ...
        {logoutMutation.isError && (
          <p role="alert" style={{ color: "oklch(45% 0.13 23.49)", fontSize: "0.75rem", marginBottom: "0.75rem" }}>
        ...
        <button
          ...
          style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", fontSize: "0.875rem", fontWeight: 700 }}
        >
          <LogOut size={16} />
